...
>
> 路径统一: 程序目录与 `HOME=/home` 对齐, 容器内统一用 `/home/.numas/` (exec/ui/extensions 三件套);
> 历史 `/root/.numas/` 镜像需重 build+push 同步. 用户工作区根仍 `/home/community`, 与程序目录正交.
...
```
容器 (ubuntu:24.04, USER root, HOME=/home, workdir /home/community)
│
├── /home/community           工作区根 (explorer 只见用户文件; 默认 workdir, NUMAS_WORKDIR 可切)
└── /home/.numas/             程序根 (镜像内置默认, 每子目录可被 -v volume 覆盖; 与 HOME 对齐)
...
- **内置默认 + 可覆盖**: 各目录镜像内带默认产物, 开箱即用; 运维 `-v host:/home/.numas/<sub>`
  覆盖即替换对应能力, 不重建镜像
- **零容器内编译**: exec / ui / extensions 全部由本地产物流程产出, 容器只 COPY + 跑
- **路径与 HOME 对齐**: 容器内所有 numas 自有路径 (exec/ui/extensions + ENV HOME=/home) 都在 `/home` 下,
  与用户工作区根 `/home/community` 同前缀; 避免 `/root` 与 `/home` 两个 root 路径割裂 (Dockerfile:50 ENV HOME=/home,
  AGENTS.md #22 解释: 程序目录硬编码 absolute path 与 HOME 解耦, 但用 /home 与 /root 是一致的 single root)
...
运行 (单进程):
  opencode web --web-ui /home/.numas/ui --registry /extensions --extensions-dir /home/.numas/extensions
...
- X.1-1 镜像构建后容器内存在 `/home/.numas/{exec,ui,extensions}` 且均含默认产物: exec/opencode 可执行、ui/ 含 index.html、extensions/ 内核 .vsix (≥3 条目)
- X.1-2 `docker run -v <host_ui_dist>:/home/.numas/ui` 后页面加载的是 host 版本 (改 index.html 标题可见) — ✅/⏳
- X.1-3 `docker run -v <host_ext>:/home/.numas/extensions` 后前端扩展列表变为 host 内容
- X.1-4 `docker run -v <host_bin>:/home/.numas/exec/opencode` 覆盖 binary 可启动 (冒烟 --version)

### 2.2 内置扩展市场 (X.2-1 ~ X.2-3)
- X.2-1 opencode 内置 `/extensions` 控制器, `curl 127.0.0.1:4096/extensions/metadata.json` 200 且条目与内置一致 (无需独立进程)
- X.2-2 页面 console 出现扩展 metadata 拉取成功 (3 entries: docxreader/html/paper), 无跨域/404
- X.2-3 容器无独立 registry 进程: `ps` 只有 tini + opencode, 无 node:7790

### 2.3 extensions 打包产物入链 + 动态添加 (X.3-1 ~ X.3-4)
- X.3-1 干净环境 (registry/vsix、registry/dist 全删) 跑 docker-build.sh, 自动完成 extensions 打包 → .vsix → 镜像内 /home/.numas/extensions 含 3 个 .vsix, metadata 3 条目
- X.3-2 容器内打开 html/paper/pdf 样例文件, 对应自定义编辑器正常渲染 (webview 资源从 /extensions 同源端点拉取)
- X.3-3 **动态添加**: 容器运行中向挂载目录放入第 4 个 .vsix (带挂载卷场景), 前端重新拉 /metadata.json 即出现新条目; 加载该扩展可用
- X.3-4 **指定目录**: opencode 以 `--extensions-dir <其它路径>` 启动 (开发/单测), metadata 内容随指定目录变化

### 2.4 workdir 正交 (X.4-1 ~ X.4-2)
- X.4-1 默认 `docker run` workdir=/home/community, UI 资源/扩展/market 均正常 (不依赖 workdir)
- X.4-2 `-e NUMAS_WORKDIR=/workspace` (容器预建) 后同样正常, explorer 根为 /workspace

### 2.5 回归 (X.5-1 ~ X.5-3)
- X.5-1 容器首启 storage 无 mkdir 500 复现 (二次刷新归零), 路径为 /home/.codeblitz (HOME=/home)
- X.5-2 `/home/.numas` 不出现在 /home/community explorer 文件树
